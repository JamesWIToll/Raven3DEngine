...
namespace Raven3DEngineCore::Events {
    class AppClock {
        ...
        std::chrono::time_point<std::chrono::system_clock> _lastRenderTime;

        //delta in seconds
        RAVEN_FLOAT _lastUpdateDelta {0.0f};
        RAVEN_FLOAT _lastRenderDelta {0.0f};
    public:
        AppClock() : _lastUpdateTime(std::chrono::system_clock::now()),
        ...
        std::chrono::time_point<std::chrono::system_clock> getLastUpdateTime() const {
            return _lastUpdateTime;
        }

        std::chrono::time_point<std::chrono::system_clock> getLastRenderTime() const {
            return _lastRenderTime;
        }

        static std::chrono::time_point<std::chrono::system_clock> getNow() {
